...
"""参数初始化"""
os.makedirs("images_lin", exist_ok=True)  # 创建images文件夹用来存放生成结果

parser = argparse.ArgumentParser()
parser.add_argument("--n_epochs", type=int, default=100, help="number of epochs of training")  # 训练epoch次数
...
for epoch in range(opt.n_epochs):  # 训练的次数就是opt.n_epochs，epoch：1个epoch表示过了1遍训练集中的所有样本。
    for i, (imgs, _) in enumerate(train_iter):
        ...
        if batches_done % opt.sample_interval == 0:
            save_image(gen_imgs.data[:25], "images_lin/%d.png" % batches_done, nrow=5, normalize=True)
